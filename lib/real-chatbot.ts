  ...
  private async callFreeAI(message: string): Promise<string> {
    try {
      ...
      if (response.ok) {
        ...
        return data.response || this.getSmartResponse(message);
      } else {
        return this.getSmartResponse(message);
      }
    } catch (error) {
      console.error('Free AI API call failed:', error);
      return this.getSmartResponse(message);
    }
  }
...
What would you like to learn about or get help with? Just ask me anything! 😊`;
  ...
  public async sendMessage(message: string): Promise<ChatResponse> {
    try {
      ...
      // Try to get response from free AI API, fallback to smart responses
      let response: string;
      try {
      ...
      const fallbackResponse = this.getSmartResponse(message);
...
What specific subject are you studying? I can give you more targeted advice!`;
  ...
  public async explainConcept(concept: string): Promise<string> {
    return this.getSmartResponse(`explain ${concept}`);
  }
